...
//
//  - 1x 4 Lines, 20 character, LCD with I2C interface board.
//    Ebay: "2004 20x4 2004A Character LCD Display Module Blue Blacklight"
//        http://www.ebay.es/itm/131282437497 4.46€
//
//  - 1x I2C serial interace for 2x16 4x20 LCD
//        http://www.ebay.es/itm/171450359303 1.26€
//
//  - 1x Ratiometric Linear Hall Effect Sensor IC A1302/A1301(newest) or AH3503(older).
//       http://www.ebay.es/itm/201117228387  0.197€ (1.97€/10pcs)
//
//
// To do:
//                   - Add selfstart rotor.
//                   - Add magnetometer function in mT/Gauss to measure magnets force.
//
// v1.33 2014-10-05  
//                   * Modified elapsed time counter only up to 999h,99',99", removed days count (999h = +41 days).
//                   * Added instantaneous rotor tachometer RPM function. Interrupt driven by change in pin 2. Recalculate rate each 1 second.
//                     - Check ratiometric linear hall effect sensor ic pdf to identify Vcc, GND and Vout pins.
//                     - Power sensor with +5V and GND from Arduino board.
//                     - Set a 10K resistor in series beetwen vout and Arduino pin 2 (to protect sensor when relay coil is set).
//                     - Adjust MAGNETS_IN_ROTOR define with the number of magnets in your rotor to get correct RPM readings.
//
// v1.32 2014-10-18  * Added elapsed time counter. Up to 49 day 12 hour (unsigned long counting ms rollover).
//                   * Added swap counter up to >>999 changes in line 3.
//
...
#define __SOFTWARE_VERSION__    "[Bedinino v1.33]"
...
#define TACHOMETER                     // Enable RPM sensing at Pin 2 with Hall efect magnetic sensor.
...
#define BATTERIES_HIGH_VOLTAGE_EVENT             15.00
#define BATTERIES_LOW_VOLTAGE_EVENT              12.00

const unsigned char swapping_batteries_number =  4;
...
unsigned long tachometertimer=0;
...
void setup(){
    ...
    // LCD Setup
      ...
      lcd.setCursor(2,0);      // Char pos 2, Line 0.
                                                // +0 flash bytes
      strcpy(message, __SOFTWARE_VERSION__);
      lcd.print(message);
      #if defined USE_SERIAL
...
*/
    ...
    #if defined TACHOMETER    
    attachInterrupt(0, tachometer_isr, FALLING); //interrupt zero (0) is on pin two(2)    
    #endif
...
// LCD Refresh
  ...
  // Swap counter
  lcd.setCursor(0,2);    
  sprintf(message, ">>%03d", swaps_counter);
  lcd.print(message);
  ...
  // Tachometer
  #if defined TACHOMETER
  if(millis()-tachometertimer > 1000){
    tachometertimer=millis();
    detachInterrupt(0);                           // Disable interrupt when calculating
    rpm = rpmcount * 60 / MAGNETS_IN_ROTOR;       // Convert frecuency to RPM.
    lcd.setCursor(13,3);    
    sprintf(message, "%04drpm", rpm);
    lcd.print(message);
    rpmcount = 0;                                 // Restart the RPM counter
    attachInterrupt(0, tachometer_isr, FALLING);   // Enable interrupt
  }
  #endif
...
// sprintf(message, "%01ud%02uh:%02u':%02u\"", (int)days, (int)hours, (int)minutes, (int)seconds);

 hours =  timeNow / hour;                                // number of hours
 minutes = (timeNow % hour) / minute ;           // and so on...
 seconds = ((timeNow % hour) % minute) / second;
 sprintf(message, "%03uh:%02u':%02u\"", (int)hours, (int)minutes, (int)seconds);
...
void BatSet(unsigned char battery, unsigned char side){
  ...
  // Latching relay control lines Setup as outputs.
  ...
  digitalWrite(RELAYS_EN_PIN, HIGH);    // Enable relays control.
 
  digitalWrite(pin, HIGH);    // Set relay, pulse on.
  delay(80);
  digitalWrite(pin, LOW);     // Set relay, pulse off.
  delay(80);      
  
  digitalWrite(RELAYS_EN_PIN, LOW);     // Disable relays control.

  // Latching relay control lines Setup as inputs for reuse.
  pinMode(BAT1_R_SECONDARY_PIN, INPUT);  // Pin 2 as a input to allow interrupt on change for ISR assisted Tachometer function.
  pinMode(BAT1_S_PRIMARY_PIN, INPUT);
  pinMode(BAT2_R_SECONDARY_PIN, INPUT);
  pinMode(BAT2_S_PRIMARY_PIN, INPUT);
  pinMode(BAT3_R_SECONDARY_PIN, INPUT);
  pinMode(BAT3_S_PRIMARY_PIN, INPUT);
  pinMode(BAT4_R_SECONDARY_PIN, INPUT);
  pinMode(BAT4_S_PRIMARY_PIN, INPUT);
...
//
#if defined TACHOMETER
void tachometer_isr(){ // this code will be executed every time the interrupt 0 (pin2) gets low.
  rpmcount++;          // will be voltatile!!
}
#endif
//
//-----------------------------------------------------
